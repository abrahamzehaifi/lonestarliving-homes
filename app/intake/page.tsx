...
function coerceIntent(v: string | null): Intent {
...
function coerceSegment(v: string | null): Segment {
  const x = (v ?? "").toLowerCase();
...
export default function IntakePage() {
  ...
  const intent = useMemo(
    () => coerceIntent(searchParams.get("type")),
    [searchParams]
  );

  const segment = useMemo(
    () => coerceSegment(searchParams.get("segment")),
    [searchParams]
  );

  const [leadType, setLeadType] = useState<LeadType>(() =>
    intentToLeadType(intent)
  );

  const [rentalGateComplete, setRentalGateComplete] = useState(false);
