...
export default function LeadFollowUp({ leadId, followUpAt }: Props) {
  ...
  useEffect(() => {
    setValue(toLocalInputValue(followUpAt));
  }, [followUpAt]);
  ...
  async function saveFollowUp(nextValue: string | null) {
    ...
    try {
      const res = await fetch("/api/lead-follow-up", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          id: leadId,
          followUpAt: normalizedNext || null,
        }),
      });
      ...
      setSuccess(normalizedNext ? "Follow-up updated." : "Follow-up cleared.");
